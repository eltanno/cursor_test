...
class GitHubAPI:
    ...
    def _make_request(
        ...
        try:
            ...
            raise GitHubAPIError(f"GitHub API request failed: {e}, Response: {response.text}")
        except Exception as e:
            raise GitHubAPIError(f"Request error: {e}")

    def _make_graphql_request(self, query: str, variables: dict | None = None) -> dict[str, Any]:
    ...
    def move_issue_to_column(
        ...
        return self._make_graphql_request(update_query, {
            "projectId": project_id,
